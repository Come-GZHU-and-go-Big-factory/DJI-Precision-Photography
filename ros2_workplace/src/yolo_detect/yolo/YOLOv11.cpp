...
YOLOv11::YOLOv11(string model_path, nvinfer1::ILogger& logger)
{
    ...
    // Constructor logic for setting dimensions
    // Note: This logic is partially redundant because init() also sets these,
    // but we keep it here to match your original structure logic.
...
void YOLOv11::init(std::string engine_path, nvinfer1::ILogger& logger)
{
    ...
    // FIXED: Added version check for TensorRT 10 compatibility
#if NV_TENSORRT_MAJOR < 10
    // TensorRT 8.x: Use Index based API
...
#else
    // TensorRT 10.x: Use Name based API
    // 1. Get Input (Index 0)
    char const* input_name = engine->getIOTensorName(0);
    nvinfer1::Dims input_dims = engine->getTensorShape(input_name);
    input_h = input_dims.d[2];
    input_w = input_dims.d[3];

    // 2. Get Output (Index 1)
    char const* output_name = engine->getIOTensorName(1);
...
{
#if NV_TENSORRT_MAJOR < 10
    // TensorRT 8.x: Pass buffer array directly
    context->enqueueV2((void**)gpu_buffers, stream, nullptr);
#else
    // TensorRT 10.x: Must explicitly bind tensor addresses
    // Note: getIOTensorName(0) assumes input is at index 0, output at index 1
    // This matches the logic used in init()
